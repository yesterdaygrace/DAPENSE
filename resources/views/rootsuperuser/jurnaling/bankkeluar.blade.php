...
<x-dashboard.page-header
    title="Jurnal Bank Keluar"
    description="Entri jurnal bank keluar"
/>
        ...
        <div class="filter-card">
            <div class="card-body">
                <form id="akun-form">
                    <div class="filter-row">
                        <div class="filter-group">
                            <label for="prefix-kode-akun" class="label">Kode Akun</label>
                            <select class="select-field" id="prefix-kode-akun" name="prefix_kode_akun" required>
                                <option value="1061">1061</option>
                                <option value="1212">1212</option>
                            </select>
                        </div>
                        <div class="flex items-end">
                            <button type="button" id="pilih-akun" class="btn-primary">Pilih</button>
                        </div>
                    </div>
                </form>
            </div>
...
<div class="card mt-4">
    ...
    <div class="card-body">
        ...
        <form id="jurnaling-form" action="{{ route('rootsuperuser/jurnaling/storebankkeluar') }}" method="POST">
            @csrf
            ...
            <div id="coa-container">
                <div class="mb-3">
                    ...
                    <div class="flex items-center gap-0">
                        <input type="text" class="input-field rounded-none w-24" maxlength="4" id="nomor_akun" name="nomor_akun" required placeholder="4 Digit Akun">
                        <span class="px-3 py-2 border border-x-0 border-gray-300 bg-gray-100 text-sm font-medium">-BK-</span>
                ...
                <div id="first-coa-group" class="flex flex-wrap gap-4 mb-3">
                    ...
                    <div class="flex-1">
                        ...
                        <input type="text" class="input-field debit-input" name="debit[]" value="" required placeholder="Masukkan Debit" oninput="this.value = this.value.replace(/[^0-9]/g, '')" oninput="toggleHiddenInput(this, 'credit')">
                    </div>
                    <div class="hidden">
                ...
                <div id="opposite-coa-group" class="flex flex-wrap gap-4 mt-4">
                    <div class="flex-1">
                        ...
                        <input class="input-field" data-list="coa-options" id="opposite-coa" oninput="updateHiddenInput(this, 'opposite-coa-id')" required placeholder="Masukkan Akun">
                    ...
                    <div class="flex-1">
                        ...
                        <input type="text" class="input-field kredit-input" name="kredit[]" value="" placeholder="Masukkan Kredit" required oninput="toggleHiddenInput(this, 'debit')">
                    </div>
                    <div class="hidden">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function calculateTotals() {
            ...
            document.getElementById('total-kredit').textContent = formatNumberValue(totalDebit);

            const kreditInput = document.querySelector('#opposite-coa-group .kredit-input');
            if (kreditInput) {
                kreditInput.value = formatNumberValue(totalDebit);
            }
        ...
        function prepareFormForEdit(dataId) {
            ...
            form.action = `/admin/jurnaling/editbk/${dataId}`;
        ...
        function prepareFormForDelete(dataId) {
            form.action = `/admin/jurnaling/deletebk/${dataId}`;
            document.getElementById('form-method').value = 'DELETE';
            form.submit();
        }

        function selectAkunKedua() {
            ...
            for (let i = 0; i < coaOptions.length; i++) {
                ...
                if (kodeAkun.startsWith(prefixKodeAkun) && kodeAkun.endsWith(nomorAkunAkhir)) {
                    document.getElementById('opposite-coa').value = option.value;
                    document.getElementById('opposite-coa-id').value = option.getAttribute('data-id');
                    break;
                }
            }
        }
        ...
        function updateNomorBukti() {
            ...
            nomorBuktiInput.value = `${nomorAkun}-BK-${nomorTransaksi}/${bulan}/${tahun}`;
        ...
        nomorTransaksiInput.addEventListener('input', function() {
            ...
            if (nomorTransaksi.length > 3 && nomorAkun.length === 4) {
                fetch(`/admin/jurnaling/cek-nomor-buktibk?nomor_transaksi=${nomorTransaksi}&nomor_akun=${nomorAkun}&tanggal_jurnal=${tanggalJurnal}`)
                    ...
                    .then(data => {
                        if (data.exists) {
                            ...
                            const nomorBukti = `${nomorAkun}-BK-${str_pad(nomorTransaksi, 4, '0', 'left')}/${bulan}/${tahun}`;
