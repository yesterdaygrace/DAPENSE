...
@section('title', 'Saldo Awal - Tambah')
@section('content')

<x-dashboard.page-header
    title="Tambah Saldo Awal"
    description="Buat saldo awal baru untuk periode akuntansi"
    :actions="'<a href=\'' . route('rootsuperuser/saldoawal') . '\' class=\'btn-secondary\'>Kembali</a>'"
/>

<div class="card">
  <div class="card-body">
    <form action="{{ route('rootsuperuser/saldoawal/store') }}" method="POST">
      @csrf
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select class="select-field @error('periode_id') border-danger @enderror" id="periode-select" name="periode_id" required>
            ...
            <option value="{{ $periode->id }}" {{ old('periode_id') == $periode->id ? 'selected' : '' }} data-start="{{ $periode->tanggal_awal }}" data-end="{{ $periode->tanggal_akhir }}">
