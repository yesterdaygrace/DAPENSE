...
<x-dashboard.page-header
    title="Pilih Bulan"
    description="Pilih bulan untuk melihat rekap Neraca Saldo"
/>

<div class="card rounded-card border border-gray-100 shadow-card mb-6">
  <div class="card-body p-4">
    <form method="GET" action="{{ route('rootsuperuser/neracasaldo/rekap', $selectedPeriode) }}" class="filter-row flex flex-wrap items-end gap-4">
      @csrf
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
  ...
  <div class="card rounded-card border border-gray-100 shadow-card flex flex-col">
    <div class="card-body flex-1 flex flex-col text-center p-5">
      ...
      <form method="GET" action="{{ route('rootsuperuser/neracasaldo/rekap', $selectedPeriode) }}" class="mt-auto">
        <input type="hidden" name="periode_id" value="{{ $selectedPeriode }}">
        <input type="hidden" name="month" value="{{ $month['id'] }}">
        <button type="submit" class="btn-primary btn-sm w-full">Lihat Rekap</button>
      </form>
    </div>
  </div>
  @endforeach
