...
<x-dashboard.page-header
    ...
    :actions="'<a href=\'' . route('admin/account/header/create') . '\' class=\'btn-primary\'>Tambah Header COA</a>'"
/>
...
<div class="card rounded-card border border-gray-100 shadow-card">
  ...
  <div class="card-body p-6">
    ...
    <x-dashboard.empty-state
        ...
        :action="'<a href=\'' . route('admin/account/header/create') . '\' class=\'btn-primary btn-sm\'>Tambah Header</a>'"
    />
    @else
          ...
          <tr class="hover:bg-gray-50/50 transition-colors">
            ...
            <td>
              <div class="flex items-center gap-2">
                <a href="{{ route('admin/account/header/edit', $headerCoa->id) }}" class="btn-warning btn-sm">Edit</a>
                <button type="button" class="btn-danger btn-sm" onclick="window.dispatchEvent(new CustomEvent('delete-modal-open', {detail: '{{ route('admin/account/header/delete', $headerCoa->id) }}'}))">Hapus</button>
              </div>
            </td>
          </tr>
          @endforeach
